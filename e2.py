...
mz = np.zeros((nx, ny), dtype=complex)
...
# Forward simulation
ex, ey, hz, ex_c, ey_c, hz_c, dt, time_src, T = fdtd.fdtd(nx=nx, ny=ny, npml=npml, res=resolution, fcen=fcen, jx=jx, jy=jy, jz=jz, mz=mz, eps=eps)

# Calculate objective function
# f = integral(E x H) = sum(weight*(EzHx - ExHz))
obj = np.real(np.sum(np.multiply(p, np.multiply(ex_c, np.conj(hz_c)))))
# f = E^2 = Ex^2 + Ey^2 + Ez^2
# obj = np.real(np.sum(np.multiply(p, np.multiply(np.conj(ex), ex)) + np.multiply(p, np.multiply(np.conj(ey), ey))))
# obj = np.real(np.sum(np.multiply(p, np.multiply(np.conj(ex_c), ex_c))))
...
# source amp
# jx_adj = iomega * np.multiply(p, np.conj(ex_c)) / amp
jx_adj = iomega * np.multiply(p, np.conj(hz_c)) / amp
...
mz_adj = -iomega * np.multiply(p, np.conj(ex_c)) / amp

print('Adjoint simulation')
# Adjoint simulation
ex_adj, ey_adj, hz_adj, _, _, _, _, _, _ = fdtd.fdtd(nx=nx, ny=ny, npml=npml, res=resolution, fcen=fcen, jx=jx_adj, jy=jy_adj, jz=jz_adj, mz=mz_adj, eps=eps)
...
for i in range(1):
    for j in range(design_region_y):
        ...
        # Forward simulation
        ex_p, ey_p, hz_p, exc_p, eyc_p, hzc_p, _, _, _ = fdtd.fdtd(nx=nx, ny=ny, npml=npml, res=resolution, fcen=fcen, jx=jx, jy=jy, jz=jz, mz=mz, eps=eps)
        ...
        # obj_p = np.real(np.sum(np.multiply(p, np.multiply(np.conj(exc_p), exc_p))))
        obj_p = np.real(np.sum(np.multiply(p, np.multiply(exc_p, np.conj(hzc_p)))))
        # Update epsilon
        design_region[i, j] -= 2 * 1e-4
        eps[start:end, start:end] = design_region

        # Forward simulation
        ex_m, ey_m, hz_m, exc_m, eyc_m, hzc_m, _, _, _ = fdtd.fdtd(nx=nx, ny=ny, npml=npml, res=resolution, fcen=fcen, jx=jx, jy=jy, jz=jz, mz=mz, eps=eps)
        ...
        # obj_m = np.real(np.sum(np.multiply(p, np.multiply(np.conj(exc_m), exc_m))))
        obj_m = np.real(np.sum(np.multiply(p, np.multiply(exc_m, np.conj(hzc_m)))))
...
plt.figure(2)
plt.scatter(range(len(g_fd)), (g_adj[:10] - g_fd) / g_fd * 100)
plt.xlabel('Points')
